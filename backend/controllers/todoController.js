...
exports.addSubTodo = async (req, res) => {
  ...
  try {
    ...
    const [todos] = await connection.execute('SELECT * FROM todos WHERE id = ?', [todoId]);
    if (todos.length === 0) {
      connection.release();
      return res.status(404).json({ message: 'Todo not found' });
    }
    ...
    // Insert subtodo with validated values
    const [result] = await connection.execute(
      'INSERT INTO subtodos (todoId, title, description, priority) VALUES (?, ?, ?, ?)',
      [todoId, title, description, priority || 'medium']
    );

    // Get updated todo with all subtodos
    const [updatedTodos] = await connection.execute('SELECT * FROM todos WHERE id = ?', [todoId]);
...
exports.updateSubTodo = async (req, res) => {
  ...
  try {
    ...
    const todoId = parseInt(req.params.id, 10);
    const subTodoId = parseInt(req.params.subTodoId, 10);
    ...
    const [subtodos] = await connection.execute('SELECT * FROM subtodos WHERE id = ? AND todoId = ?', [subTodoId, todoId]);
    ...
    const [allSubtodos] = await connection.execute('SELECT * FROM subtodos WHERE todoId = ?', [todoId]);
    if (allSubtodos.length > 0) {
      const allCompleted = allSubtodos.every(st => st.completed);
      await connection.execute('UPDATE todos SET completed = ? WHERE id = ?', [allCompleted, todoId]);
    }
